...
<div
    x-data="{
        ...
        /* ── to-do list ─────────────────────────── */
        get todoTasks() {
            return [...this.tasks].sort((a,b)=>a.dueDate.localeCompare(b.dueDate));
        },
        formatDue(ymd) {
        ...
        priorityDot(priority) {
            return { Urgent:'#ef4444', Important:'#ec4899', Medium:'#3b82f6', Low:'#6b7280' }[priority] ?? '#9ca3af';
        },
    }"
    ...
    <div class="w-80 shrink-0 flex flex-col overflow-hidden shadow-lg">
        ...
        <div class="bg-gray-50 rounded-b flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex items-center justify-between px-8 py-4 shrink-0">
                <p class="text-base font-normal text-gray-800">To-do</p>
                <button class="w-6 h-6 rounded flex items-center justify-center text-gray-500 hover:bg-gray-200 text-xl leading-none">+</button>
            </div>

            {{-- Scrollable list --}}
            <div class="flex-1 overflow-y-auto px-8 pb-6">
                <template x-if="todoTasks.length===0">
                    <p class="text-sm text-gray-400 text-center py-6">No tasks found.</p>
                </template>

                <template x-for="task in todoTasks" :key="task.id">
                    <div class="flex items-center justify-between border-b border-gray-200 py-3 gap-3 cursor-pointer hover:opacity-80 transition"
                         @click="goToTask(task)">
                        <div class="flex flex-col gap-0.5 min-w-0">
                            <p class="text-sm text-gray-800 truncate" x-text="task.title"></p>
                            <p class="text-xs text-gray-400" x-text="'Due: ' + formatDue(task.dueDate)"></p>
                        </div>
                        <div class="flex items-center gap-1 shrink-0">
                            <span class="w-1.5 h-1.5 rounded-full" :style="'background:'+priorityDot(task.priority)"></span>
